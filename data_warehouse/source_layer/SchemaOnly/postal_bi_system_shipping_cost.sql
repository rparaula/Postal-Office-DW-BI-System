...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`ryan`@`%`*/ /*!50003 TRIGGER `trg_shipping_cost_bi_validate_nonnegative` BEFORE INSERT ON `shipping_cost` FOR EACH ROW BEGIN
    IF NEW.`actual_shipping_charge` IS NULL OR NEW.`actual_shipping_charge` < 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'shipping_cost.actual_shipping_charge must be nonnegative.';
    END IF;

    IF NEW.`material_cost` IS NULL OR NEW.`material_cost` < 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'shipping_cost.material_cost must be nonnegative.';
    END IF;

    IF NEW.`transportation_cost` IS NULL OR NEW.`transportation_cost` < 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'shipping_cost.transportation_cost must be nonnegative.';
    END IF;
END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`ryan`@`%`*/ /*!50003 TRIGGER `trg_shipping_cost_bu_validate_nonnegative` BEFORE UPDATE ON `shipping_cost` FOR EACH ROW BEGIN
    IF NEW.`actual_shipping_charge` IS NULL OR NEW.`actual_shipping_charge` < 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'shipping_cost.actual_shipping_charge must be nonnegative.';
    END IF;
...
-- Dump completed on 2026-06-21 14:45:16
