...
CREATE TABLE `incident` (
  ...
  `resolved_by_employee_id` int DEFAULT NULL,
  ...
  KEY `fk_incident_resolved_by_employee` (`resolved_by_employee_id`),
  ...
  CONSTRAINT `fk_incident_resolved_by_employee` FOREIGN KEY (`resolved_by_employee_id`) REFERENCES `employee` (`employee_id`) ON DELETE SET NULL ON UPDATE CASCADE,
...
) ENGINE=InnoDB AUTO_INCREMENT=6293 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
/*!50003 CREATE*/ /*!50017 DEFINER=`ryan`@`%`*/ /*!50003 TRIGGER `trg_incident_bi_validate_package_movement` BEFORE INSERT ON `incident` FOR EACH ROW BEGIN
    DECLARE v_movement_package_id BINARY(16);
    ...
    IF NEW.`package_movement_id` IS NOT NULL THEN
        ...
        IF v_movement_package_id IS NULL THEN
            SIGNAL SQLSTATE '45000'
                SET MESSAGE_TEXT = 'incident.package_movement_id does not reference a valid package_movement row.';
        END IF;

        IF NEW.`package_id` IS NULL THEN
            SET NEW.`package_id` = v_movement_package_id;
        ELSEIF NOT (NEW.`package_id` <=> v_movement_package_id) THEN
            SIGNAL SQLSTATE '45000'
    ...
    IF NEW.`package_id` IS NOT NULL THEN
        ...
        IF v_package_received_date IS NOT NULL
           AND NEW.`incident_date` < v_package_received_date THEN
            SIGNAL SQLSTATE '45000'
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`ryan`@`%`*/ /*!50003 TRIGGER `trg_incident_bu_set_resolved_at` BEFORE UPDATE ON `incident` FOR EACH ROW BEGIN
    DECLARE v_new_is_closed_status TINYINT(1) DEFAULT 0;

    SELECT COALESCE(ist.`is_closed_status`, 0)
    INTO v_new_is_closed_status
    FROM `incident_status` ist
    WHERE ist.`incident_status_id` = NEW.`incident_status_id`;

    IF v_new_is_closed_status = 1
       AND NEW.`resolved_at` IS NULL
       AND (
            OLD.`incident_status_id` <> NEW.`incident_status_id`
            OR OLD.`incident_status_id` IS NULL
       ) THEN
...
-- Dump completed on 2026-06-21 14:45:42
