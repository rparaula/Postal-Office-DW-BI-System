...
/*!50003 CREATE*/ /*!50017 DEFINER=`ryan`@`%`*/ /*!50003 TRIGGER `beforeInsertShippingdetailsValidateBusinessRules` BEFORE INSERT ON `shippingdetails` FOR EACH ROW BEGIN
  ...
  DECLARE v_has_active_service_type INT DEFAULT 0;
  ...
  SELECT
    COUNT(*),
    COALESCE(MAX(CASE WHEN st.`is_active` = 1 THEN 1 ELSE 0 END), 0)
  INTO v_package_exists, v_has_active_service_type
  FROM `package` p
  ...
  IF v_has_active_service_type = 0 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'shippingdetails.package_id must reference a package with an active service_type.';
  END IF;
...
-- Dump completed on 2026-06-21 14:45:41
